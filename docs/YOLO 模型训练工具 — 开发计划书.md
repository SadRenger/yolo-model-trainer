...
<!-- meta
affects: [状态码 R-xxx号段, 测试计划 第3-5章]
说明: 技术栈版本、模块参数格式、架构变更影响状态码定义和测试策略。注意：计划书优先级最低（冲突时以实际环境为准）。
-->
...
| 深度学习框架 | PyTorch + Ultralytics | PyTorch == 2.6.0+cu124, Ultralytics == 8.4.71 |
| 安装包 | NSIS | ≥ 3.0 |
...
| 维度 | 估算 |
|:---|:---|
| 总开发周期 | 12 周（不含需求确认和设计阶段，初始预估，实际进度以文档定稿和代码提交为准） |
| 前端页面数 | 4 个主页面 + 弹窗/Toast 等全局组件 |
| Python 后端模块 | 5 个独立模块 |
| Rust 后端模块 | 进程管理、事件总线、文件系统、环境检测等 |
| 安装包体积目标 | 约 2.0~2.4 GB（安装后约占用 4.5 GB 磁盘空间，建议预留 8 GB） |
...
```
阶段一: 工程搭建        (Week 1)
阶段二: Python 引擎     (Week 2–4)
阶段三: Rust 后端       (Week 4–6)
阶段四: 前端 UI         (Week 6–8)
阶段五: 联调集成        (Week 8–10)
阶段六: 打包与测试      (Week 10–12)
```
...
> 设计风格指南已完整定义了所有颜色、字体、间距、组件样式。本计划书不重复列出。所有视觉规范以《YOLO 模型训练工具 — 设计风格指南》为准。CSS 变量参考设计风格指南 5.2 节。
...
| 8.8 | 安装包体积优化 | 精简不必要的 PyTorch 组件，目标 2.0~2.4 GB |
| 8.9 | 兼容性测试（≥5台） | 在 5 台以上不同配置 Windows 机器上完成测试（含 Win10/Win11、GPU/CPU-only、不同显卡型号） |
| 8.10 | 示例数据集准备 | 小型 YOLO 标注数据集，含 3 个类别，约 50 张图片 |
| 8.11 | 用户使用说明编写 | 单页 HTML 文档，内容：安装 → 下载模型 → 数据集准备要求 → 第一次训练 → 推理测试 |
| 8.12 | GitHub 仓库准备 | README.md（中英双语、截图、快速开始）、MIT LICENSE、CONTRIBUTING.md |
| 8.13 | CI/CD 配置 | GitHub Actions 自动构建，产物发布到 GitHub Releases |
...
| 训练中暂停恢复 | 开始训练 → 等到第 3 轮 → 暂停 → 等待 30 秒 → 继续训练 → 训练正常完成 | 暂停期间参数区锁定，继续后无缝恢复训练 |
| 训练中关闭程序 | 训练到第 3 轮 → 点击关闭按钮 → 确认退出 → 重新打开 → 训练历史中有该任务 → 点击断点续训 → 加载权重 + 新参数从 epoch 0 开始 | last.pt 权重被保留，断点续训成功 |
...
| 风险 | 概率 | 影响 | 缓解措施 |
|:---|:---|:---|:---|
| PyTorch 嵌入打包体积过大 | 低 | 安装包超 2.4 GB | 已按实测数据修正体积预估，预留 0.4 GB 余量 |
| NVIDIA 驱动兼容性 | 中 | 部分用户 GPU 无法使用 | 自动降级到 CPU 训练并给出明确提示 |
| Ultralytics API 变更 | 中 | 训练/推理功能异常 | 锁定 `ultralytics==8.4.71`，升级前在独立分支完整回归 |
| Python 子进程管理 | 低 | 进程泄露导致资源占用 | 超时强制 kill，进程树完整清理 |
| 训练进程异常崩溃 | 中 | 用户训练进度丢失 | 每轮自动保存 last.pt；崩溃后训练历史标记为可续训状态 |
| Windows 不同版本 WebView2 表现不一致 | 低 | 前端渲染异常 | 安装包内嵌 WebView2 引导安装，兼容 Win10 1803+ |
