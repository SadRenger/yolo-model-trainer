...
- 阶段二收尾完成（数据集预览 + 全部 BUG 清零）
- 阶段三推进中：
  - 训练历史真实数据 ✅ (Rust 写 history.json, get_task_history 读)
  - 下载模型 ✅ (save_file_dialog + copy_file)
  - 查看报告 ✅ (open_report → 默认浏览器)
  - 删除任务 ✅ (delete_task 同时删记录+文件)
  - 搜索/筛选/排序 ✅ (纯前端实时过滤)
  - 侧边栏三态 ✅ (新建训练/训练中%/训练完成, 全局监听不丢)
  - 停止训练文件信号 ✅ (sys.exit(2) + train:stopped)
  - 训练中切页保持 ✅ (进度%/日志/校验状态全部缓存)

## 下一步（阶段三继续）

1. 断点续训（从历史恢复配置 → 新建训练预填）
2. 批量删除
3. 暂停/恢复（文件信号完善）
