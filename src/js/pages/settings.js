/* ═══════════════════════════════════════════════════
   YOLO Model Trainer — Page: Settings (global namespace)
   ═══════════════════════════════════════════════════ */
(function() {
  var App = window.App;
  App.pages = App.pages || {};
    ...
    mount: function(container, params) {
      ...
      configCard.innerHTML =
        ...
        '<div class="card__body">' +
          ...
          '</div>' +
        '</div>' +
        '<div class="card__footer"><button class="btn btn--primary btn--sm">保存设置</button></div>';
      page.appendChild(configCard);

      // System Environment Card
      var envCard = document.createElement('div');
      envCard.className = 'card page-section';
      envCard.innerHTML =
        '<div class="card__header"><span class="card__header-icon">💻</span><h2 class="card__title">系统环境</h2><button class="btn btn--ghost btn--sm" id="btn-refresh-env" style="margin-left:auto">🔄 重新检测</button></div>' +
        '<div class="card__body" id="env-status-list"><div class="empty-state"><div class="spinner"></div><p style="color:var(--text-muted);margin-top:8px">检测中…</p></div></div>';
      page.appendChild(envCard);

      // Model Download Guide Card
      var guideCard = document.createElement('div');
      guideCard.className = 'card page-section';
      guideCard.innerHTML =
        '<div class="card__header"><span class="card__header-icon">📖</span><h2 class="card__title">模型下载指引</h2></div>' +
        '<div class="card__body">' +
          '<p style="color:var(--text-secondary);margin-bottom:12px">请从以下官方地址下载 YOLO 预训练权重文件（.pt 格式）：</p>' +
          '<h3 style="font-size:var(--fs-body);color:var(--text-primary);margin-bottom:8px">YOLOv8 / YOLOv11</h3>' +
          '<p style="color:var(--text-secondary);margin-bottom:8px"><a href="https://github.com/ultralytics/assets/releases" target="_blank">https://github.com/ultralytics/assets/releases</a></p>' +
          '<h3 style="font-size:var(--fs-body);color:var(--text-primary);margin:16px 0 8px">版本选择建议</h3>' +
          '<table class="data-table"><thead><tr><th>版本</th><th>参数量</th><th>适用数据量</th><th>推荐场景</th></tr></thead><tbody>' +
            ...
            '<tr><td>m (medium)</td><td>25.9M</td><td>5,000~20,000</td><td>精度与速度平衡</td></tr>' +
            '<tr><td>l (large)</td><td>43.7M</td><td>20,000~100,000</td><td>高精度要求</td></tr>' +
            '<tr><td>x (xlarge)</td><td>68.2M</td><td>&gt;100,000</td><td>最高精度</td></tr>' +
          '</tbody></table>' +
        '</div>';
